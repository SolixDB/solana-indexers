...
interface NodeConnectionMonitorOptions {
...
export class NodeConnectionMonitor {
    ...
    return new Promise((resolve, reject) => {
      if (this.state === "OPEN") {
        ...
        if (sinceLastFailure < this.recoveryTimeout) {
          console.warn("[WARN] Circuit breaker is OPEN. Skipping connection attempt.");
          return resolve();
        }
        console.info("[INFO] Circuit breaker moving to HALF_OPEN state.");
